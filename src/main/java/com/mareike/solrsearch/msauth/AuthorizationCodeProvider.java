...
public class AuthorizationCodeProvider extends BaseAuthentication implements IAuthenticationProvider, ICoreAuthenticationProvider{
    ...
    public AuthorizationCodeProvider(
        ...
        super(	scopes,
                clientId,
                GetAuthority(nationalCloud == null? NationalCloud.Global: nationalCloud, tenant == null? Constants.Tenants.Common: tenant),
                redirectUri,
                nationalCloud == null? NationalCloud.Global: nationalCloud,
                tenant == null? Constants.Tenants.Common: tenant,
                clientSecret);
    ...
    public Request authenticateRequest(Request request) {
        String tokenParameter =  Constants.BEARER + getAccessTokenSilent();
        return request.newBuilder().addHeader(Constants.AUTHORIZATION_HEADER, tokenParameter).build();
    }

    @Override
    public void authenticateRequest(IHttpRequest request) {
        String tokenParameter =  Constants.BEARER + getAccessTokenSilent();
        request.addHeader(Constants.AUTHORIZATION_HEADER, tokenParameter);
    }
